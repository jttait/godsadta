...
func TestShouldInstantiateMaxHeap(t *testing.T) {
	m := arraymaxheap.NewMaxHeapArray[int]()
	if m == nil {
...
func TestShouldExtractFromMaxHeapWithTwoItems(t *testing.T) {
	m := arraymaxheap.NewMaxHeapArray[int](5, 15, 10)
	result, _ := m.Extract()
	assert.AssertEqual(result, 15, t)
}

func TestShouldExtractMultipleTimesFromMaxHeap(t *testing.T) {
	m := arraymaxheap.NewMaxHeapArray[int](78, 39, 24, 98, 72, 77, 71, 68, 53, 41, 60, 54, 92, 36, 75, 47, 33, 80, 9, 61)
	result, _ := m.Extract()
...
func TestShouldBeSizeTwoForMaxHeapWithTwoItems(t *testing.T) {
	m := arraymaxheap.NewMaxHeapArray[int](1, 2)
	result := m.Size()
